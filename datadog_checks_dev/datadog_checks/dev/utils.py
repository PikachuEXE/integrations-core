...
def run_command(command, hide=None, encoding='utf-8', shell=False):
    if shell == 'detect':
        shell = NEED_SHELL
    ...
    # TODO: Only create temp files when needed.
    with TemporaryFile() as stdout_file, TemporaryFile() as stderr_file:
        if hide:
            if hide is True or hide == 'both':
                stdout, stderr = stdout_file, stderr_file
            elif hide in ('stdout', 'out'):
                stdout, stderr = stdout_file, None
            elif hide in ('stderr', 'err'):
                stdout, stderr = None, stderr_file
            else:
                raise ValueError('Unknown hiding method `{}`'.format(hide))
        else:
        ...
        if stdout is None:
            ...
            stdout_file.seek(0)
            stdout = stdout_file.read().decode(encoding)

        if stderr is None:
            stderr = ''
        else:
            stderr_file.seek(0)
            stderr = stderr_file.read().decode(encoding)
